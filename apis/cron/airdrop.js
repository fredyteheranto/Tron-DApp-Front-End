    ...
    rp(options)
        .then((response) => {
            ...
            Promise.all(promisesArray)
                .then(async (promises_response) => {
                    console.log(promises_response);
...
async function sendEHRTokensToAirDropUsers(to, amount) {
    ...
    try {
        ...
        [err, obj] = await utils.to(db.models.transections.create(
            { address: process.env.MAIN_ACCOUNT_ADDRESS_KEY, number_of_token: amount, trx_hash: trxId, type: 'Sent' },
        ));
        [err, obj] = await utils.to(db.models.air_drop_users.create(
            { tron_user_address: to, reward_amount: amount },
        ));
    } catch (error) {
        console.log(error);
    }
